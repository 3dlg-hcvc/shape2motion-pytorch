...
def compute_anchor_pts_loss(pred_anchor_pts, gt_anchor_pts, mask):
    ...
    anchor_pts_loss = torch.mean(
        F.cross_entropy(pred_anchor_pts, gt_anchor_pts.long(), reduction='none') * (mask * (num_neg / num_pos) + 1))
    true_anchor_pos = torch.sum(
        torch.eq(torch.argmax(pred_anchor_pts, axis=1).int(), gt_anchor_pts.int()).float() * mask, axis=1)
...
def compute_joint_direction_cat_loss(pred_joint_direction_cat, gt_joint_direction_cat, mask):
    ...
    joint_direction_cat_loss = torch.mean(
        torch.sum(F.cross_entropy(pred_joint_direction_cat, gt_joint_direction_cat.long(), reduction='none') * mask,
                  axis=1) / mask_sum)
    joint_direction_cat_accuracy = torch.mean(torch.sum(
        torch.eq(torch.argmax(pred_joint_direction_cat, axis=1).int(), gt_joint_direction_cat.int()).float() * mask,
        axis=1) / mask_sum)
...
def compute_joint_direction_reg_loss(pred_joint_direction_reg, gt_joint_direction_reg, mask):
    joint_direction_reg_loss = torch.mean(torch.sum(
        torch.mean(F.smooth_l1_loss(pred_joint_direction_reg, gt_joint_direction_reg, reduction='none'), axis=2) * mask,
        axis=1) / torch.sum(mask, axis=1))
    return joint_direction_reg_loss


def compute_joint_origin_reg_loss(pred_joint_origin_reg, gt_joint_origin_reg, mask):
    joint_origin_reg_loss = torch.mean(torch.sum(
        torch.mean(F.smooth_l1_loss(pred_joint_origin_reg, gt_joint_origin_reg, reduction='none'), axis=2) * mask,
        axis=1) / torch.sum(mask, axis=1))
    return joint_origin_reg_loss


def compute_joint_type_loss(pred_joint_type, gt_joint_type, mask):
    joint_type_loss = torch.mean(
        torch.sum(F.cross_entropy(pred_joint_type, gt_joint_type.long(), reduction='none') * mask, axis=1) / torch.sum(
            mask, axis=1))
    joint_type_accuracy = torch.mean(
        torch.sum(torch.eq(torch.argmax(pred_joint_type, axis=1).int(), gt_joint_type.int()).float() * mask,
                  axis=1) / torch.sum(mask, axis=1))
...
def compute_confidence_loss(pred_confidence, pred_simmat, gt_simmat, threshold, epsilon):
    ...
    pts_iou = torch.sum(torch.logical_and(pred_positive, gt_positive).float(), axis=2) / (
                torch.sum(torch.logical_or(pred_positive, gt_positive).float(), axis=2) + epsilon)
...
def compute_motion_scores_loss(pred_motion_scores, gt_motion_scores, mask, epsilon):
    motion_scores_loss = torch.mean(
        torch.sum(torch.mean(F.smooth_l1_loss(pred_motion_scores, gt_motion_scores, reduction='none'), axis=2) * mask,
                  axis=1) / (torch.sum(mask, axis=1) + epsilon))
...
def compute_part_proposal_loss(pred_part_proposal, gt_part_proposal, epsilon):
    ...
    iou = torch.mean(torch.sum(torch.logical_and(proposal, gt_part_proposal), axis=-1) / (
                torch.sum(torch.logical_or(proposal, gt_part_proposal), axis=-1) + epsilon))
...
def compute_motion_regression_loss(pred_motion_regression, gt_motion_regression):
    motion_regression_loss = torch.mean(
        torch.sum(F.smooth_l1_loss(pred_motion_regression, gt_motion_regression, reduction='none'), axis=1))
